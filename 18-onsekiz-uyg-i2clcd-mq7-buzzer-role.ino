...
const int mq7_pini = A0;    // MQ7 Karbon monoksit (CO) sensörü pin bağlantısı
...
void loop() 
{
  ...
  int okunan_co_bilgisi = analogRead(mq7_pini);   // Karbon monoksit (CO) sensöründen okuma yap
 
  int tehlike_durumu = 200; //  // LPG seviyesinin tehlikeli noktaya geldiği anı belirledik.
  
  if (okunan_co_bilgisi >= tehlike_durumu) 
  {
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("CO Degeri:");
    lcd.setCursor(13, 0);
    lcd.print(okunan_co_bilgisi);
    lcd.setCursor(0, 1);
    lcd.print("Dikkat CO Tespiti");
    buzz(1500, 200);
