...
void setup()
 {
  ...
  lcd.print("i2clcd-pot-mtr");     // "Hello World!" yazısını ekrana yaz
...
void loop() 
{
  ...
  pot_degeri = analogRead(pot_pin);  // Potansiyometre değerini oku
  pwm_hizi = map(pot_degeri, 0, 1023, 0, 255);  //ADC 0 - 1023 to 0-255 pwm 
  ...
  lcd.setCursor(0, 0);           // İlk satırın başına git
  lcd.print("Pot D:");           // "Pot D:" yazısını ekrana yaz
  lcd.setCursor(8, 0);           // İlk satırın 8. hücresine git
  lcd.print(pot_degeri);         // pot degerini ekrana yaz
  ...
  delay(250);  // 250 milisaniye bekleme
