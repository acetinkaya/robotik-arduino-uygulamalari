...
const int mq8_pini = A0;    // MQ8 Hidrojen sensörü pin bağlantısı
...
void loop() 
{
  ...
  int okunan_hidrojen_bilgisi = analogRead(mq8_pini);   // Hidrojen sensöründen okuma yap
 
  int tehlike_durumu = 350; //  // LPG seviyesinin tehlikeli noktaya geldiği anı belirledik.
  
  if (okunan_hidrojen_bilgisi >= tehlike_durumu) 
  {
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("Hidrojen D:");
    lcd.setCursor(13, 0);
    lcd.print(okunan_hidrojen_bilgisi);
    lcd.setCursor(0, 1);
    lcd.print("Dikkat Hidrojen!!");
    buzz(1500, 200);
