...
const int mq5_pini = A0;    // MQ5 LPG sensörü pin bağlantısı
...
void loop() 
{
  ...
  int okunan_lpg_bilgisi = analogRead(mq5_pini);   // MQ3 Gaz sensöründen okuma yap
 
  int tehlike_durumu = 100; //  // LPG seviyesinin tehlikeli noktaya geldiği anı belirledik.
  
  if (okunan_lpg_bilgisi >= tehlike_durumu) 
  {
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("Dogalgaz D:");
    lcd.setCursor(13, 0);
    lcd.print(okunan_lpg_bilgisi);
    lcd.setCursor(0, 1);
