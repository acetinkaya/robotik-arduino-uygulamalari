...
const int mq9_pini = A0;    // MQ9 Metan gazı sensörü pin bağlantısı
...
void loop() 
{
  ...
  int okunan_metan_bilgisi = analogRead(mq9_pini);   // MQ9 Metan gaz sensöründen okuma yap
 
  int tehlike_durumu = 350; //  // LPG seviyesinin tehlikeli noktaya geldiği anı belirledik.
  
  if (okunan_metan_bilgisi >= tehlike_durumu) 
  {
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("Metan D: ");
    lcd.setCursor(13, 0);
    lcd.print(okunan_metan_bilgisi);
    lcd.setCursor(0, 1);
    lcd.print("Dikkat Metan!!");
    buzz(1500, 200);
